...
class HadithDocumentUploadAdmin(admin.ModelAdmin):
    ...
    def get_urls(self):
        ...
        custom_urls = [
            ...
            path('reprocess/<int:doc_id>/', self.admin_site.admin_view(self.reprocess_document),
                 name='hadith_hadithdocumentupload_reprocess'),
            path('list-documents/', self.admin_site.admin_view(self.list_documents),
                 name='hadith_hadithdocumentupload_list_documents'),
        ]
        return custom_urls + urls

    def list_documents(self, request):
        from .models import HadithDocumentUpload
        docs = HadithDocumentUpload.objects.all().order_by('-uploaded_at')
        data = [{'id': d.id, 'name': str(d), 'uploaded_at': d.uploaded_at.isoformat()} for d in docs]
        return JsonResponse(data, safe=False)

    def reprocess_document(self, request, doc_id):
        from .models import HadithDocumentUpload
        if not request.user.is_superuser:
            return JsonResponse({'error': 'Permission denied'}, status=403)
        doc = get_object_or_404(HadithDocumentUpload, pk=doc_id)
        parsed = self._parse_upload_document(doc.file)
        if parsed is None:
            return JsonResponse({
                'error': 'This document has more hadiths than available in the chapter. Cannot reprocess.'
            }, status=400)
